...
def title(_url_=''):
    ...
    response = requests.get(_url_, headers={"User-Agent": "Mozilla/5.0"})  # response객체
    soup = BeautifulSoup(response.text, "html.parser")  # parsing ,  response객체에서 .text로 html을 추출해서(string) bs4로 parsing합니다.
    head_line = soup.select('.headline')  # bs4객체의 메소드 slect사용, css class로 검색할 수 있습니다. class="headline" 제목 클래스 모두 찾기
...
def date(_url_=''):
    ...
    response = requests.get(_url_, headers={"User-Agent": "Mozilla/5.0"})  # response객체
    soup = BeautifulSoup(response.text, "html.parser")  # parsing ,  response객체에서 .text로 html을 추출해서(string) bs4로 parsing합니다.
    head_line = soup.select_one('em')  # bs4객체의 메소드 slect사용, tag로 검색할 수 있습니다. tag = <em> 입력날짜, 수정날짜 강조하는 tag에서 첫번째 입력날짜만 추출합니다.
    resoup_ = BeautifulSoup(str(head_line), "html.parser")  # 다시 parsing
...
press_url = click_press('경향신문')  # 경향신문 클릭
pages_ = page_count(press_url)  # 경향신문의 페이지 수 Return
...
result_df.to_csv("oasis_kyunghyan.csv")  # csv로 바꿔서 저장합니다.
